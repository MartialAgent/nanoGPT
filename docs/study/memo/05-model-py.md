...
1. **진짜 GPT-2 어휘는 50257개.** `50304`는 그것을 **64의 배수로 올림한 패딩값**이다.
2. 늘어난 47칸에는 **대응하는 토큰이 없다.** 토크나이저는 여전히 `0~50256`만 출력한다.
   토큰을 새로 정의한 게 아니라 **모델 쪽 텐서만 크게 잡은 것**이다.
3. 연산량은 오히려 **늘어나는데도 더 빠르다.** 이유는 GPU 정렬(§왜 빨라지는가).
...
> "GPT-2 기본 어휘가 그럼 50304개인건가?"
> "openai에서 공개한 사전 수가 있는데 아무 것도 안 넣고 빈 값으로 숫자 올려서 학습에 최적화했다, 맞나?"
> "이걸 용어로 뭐라고 부르지?"

`prepare.py` 두 종류(char / BPE)의 산출물 차이를 보다가, `meta.pkl`이 없을 때 train.py가 떨어지는
기본값 `50304`가 어디서 온 숫자인지에서 출발한 질문.

**헷갈린 지점**

1. **"BPE는 model.py에서 쓴다" — 아니다.**
   `tiktoken.get_encoding("gpt2")` 호출은 [`data/shakespeare/prepare.py:22`](../../data/shakespeare/prepare.py#L22) **한 곳뿐**이다.
   `model.py`는 토크나이저가 무엇인지 전혀 모른다. `vocab_size`라는 **정수 하나만** 받는다.
   그래서 같은 `model.py`가 char(65)든 BPE(50304)든 그대로 돌아간다.

2. **"양자화"라는 단어의 충돌.**
   여기서의 tile/wave quantization은 int8·fp8 같은 **모델 양자화(quantization)와 완전히 무관한 동음이의어**다.
   검색할 때 섞이기 쉽다.

3. **"padding token"이라 부르면 안 된다.**
   NLP의 `<pad>`는 배치 길이를 맞추는 **실재하는 특수토큰**이다. 여기 47칸은 토크나이저에 없는 자리라 성격이 다르다.

4. **패딩은 from-scratch 전용이다.**
   사전학습 GPT-2 체크포인트를 불러올 땐 OpenAI 가중치 shape과 맞아야 하므로
   [`model.py:222-223`](../../model.py#L222-L223)에서 **50257로 강제**된다.

5. **`uint16` 저장 한계.**
   `prepare.py`는 토큰을 `np.uint16`으로 저장한다. vocab_size가 65535를 넘는 토크나이저에는
   이 코드 그대로는 못 쓴다. (GPT-2 50257은 안전, char 65는 여유)
